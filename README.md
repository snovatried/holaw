...
- **Medicamentos:** alta manual y autocompletado desde catálogo de Ecuador (solo formas comestibles).
...
- Si ves dos puertos (`smtp_port` y `SMTP_PORT`), con `sendmail_path=msmtp` el que realmente usa el envío es `SMTP_PORT`; `smtp_port` de php.ini queda solo como referencia.
